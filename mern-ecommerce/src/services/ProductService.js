...
import { axiosJWT } from "./UserService";
  ...
  export const getAllProduct = async (search, limit) => {
    ...
    if (search?.length > 0) {
      res = await axios.get(
        `${process.env.REACT_APP_API_URL}/product/getAll?filterField=name&filterOrder=${search}&limit=${limit}`
      );
    } else {
      res = await axios.get(
...
export const getTypeProduct = async (type, page, limit) => {
  if (type) {
    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/product/getAll?filterField=type&filterOrder=${type}&page=${page}&limit=${limit}`
    );
    return res.data;
  }
};
